...
function normalizeKey(key, throwOnError = true) {
  let value = String(key || '').trim().replace(/\s+/g, '')
...
function normalizeEndpoint(endpoint) {
  ...
  try {
    ...
    throw new Error('Endpoint must be a valid URL, for example https://api.tinyapi.org.')
  }

  const isTinyApi = parsed.protocol === 'https:' && /(^|\.)tinyapi\.org$/i.test(parsed.hostname)
  const isLocal =
...
async function askEndpoint(label, fallbackEndpoint) {
  while (true) {
    const value = await ask(`${label} (Enter ใช้ค่าในวงเล็บ / paste URL ได้)`, fallbackEndpoint)
    if (/^[1-9]$/.test(String(value).trim())) {
      warn('ตอนนี้เป็นช่อง Endpoint ไม่ใช่เมนูหลัก ถ้าจะใช้ค่าเดิมให้กด Enter หรือวาง URL endpoint')
      continue
    }
    ...
    try {
      ...
      warn(error.message)
      warn('ตัวอย่างที่ถูกต้อง: https://api.tinyapi.org หรือ http://127.0.0.1:3000')
    }
  }
}
...
function showThaiGuide() {
  ...
  say('  Endpoint: กด Enter เพื่อใช้ค่าเดิม หรือวาง URL เช่น http://127.0.0.1:3000')
...
function loadState() {
  ...
  return {
    endpoint: state.endpoint || process.env.TINYTOKEN_ENDPOINT || DEFAULT_ENDPOINT,
...
function detectKey() {
  ...
  const claudeFull = normalizeKey(claudeSettings?.env?.ANTHROPIC_API_KEY, false)
...
async function collectBasics(toolName, modelKey) {
  ...
  const model = await ask('Model name', state.models[modelKey] || DEFAULT_MODELS[modelKey])
...
async function maybeInstall(pkg, name) {
  const shouldInstall = await askYesNo(`Install ${name} globally with npm?`, true)
...
function persistEnv(vars) {
  ...
  if (shell === 'bash' && process.platform === 'darwin') rc = path.join(home, '.bash_profile')
  ...
  const block = [
    mark,
    ...Object.entries(vars).map(([key, value]) => `export ${key}=${JSON.stringify(value)}`),
...
function updateClaudeFiles({ endpoint, key, model }) {
  ...
  settings.customApiKeyResponses = approveKey(settings.customApiKeyResponses, key)
  settings.model = model
  settings.permissions ??= {
    allow: ['Edit', 'Read', 'Write', 'Bash', 'Agent', 'Glob', 'Grep', 'WebSearch', 'WebFetch'],
    deny: [],
    ask: [],
...
function updateGeminiFiles({ endpoint, key, model }) {
  ...
  warn('Official Gemini CLI may not fully support custom TinyToken base URL. Use the Gemini-compatible endpoint with clients that allow base URL override.')
...
async function configureGemini() {
  ...
  const install = await askYesNo('Install official @google/gemini-cli? (TinyToken base URL support may depend on client version)', false)
...
async function configureOther() {
  const values = await collectBasics('OpenAI-compatible / Other tools', 'openai')
  updateOtherFiles(values)
}

async function changeDefaults() {
  const state = loadState()
  state.endpoint = await askEndpoint('Default endpoint / Base URL', state.endpoint)
  ...
  state.models.openai = await ask('Default OpenAI-compatible model', state.models.openai)
  saveState(state)
  ok('defaults saved')
}
