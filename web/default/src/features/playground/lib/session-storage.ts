...
const DATABASE_NAME = 'tinyapi-playground'
...
function openPlaygroundDatabase() {
...
async function getAllSessions(database: IDBDatabase) {
  return new Promise<PlaygroundSession[]>((resolve, reject) => {
    const transaction = database.transaction(SESSION_STORE, 'readonly')
    const request = transaction.objectStore(SESSION_STORE).getAll()

    request.onsuccess = () => {
      resolve(
        (request.result as PlaygroundSession[])
          .map((session) => ({
            ...session,
...
export async function loadPlaygroundSessions() {
...
export async function savePlaygroundSession(session: PlaygroundSession) {
  const database = await openPlaygroundDatabase()
  try {
